...
namespace Best.HTTP.Caching
{
    ...
    public class HTTPCache : IDisposable, IHeartbeat
    {
        ...
        public void EndCache(HTTPCacheContentWriter cacheResult, bool completedWithoutIssue, LoggingContext context)
        {
            ...
            {
                _database.ReleaseWriteLock(hash, cacheResult.ProcessedLength, false, cacheResult.Context);
        ...
        public Stream BeginReadContent(Hash128 hash, LoggingContext context)
        {
            ...
            {
                HTTPManager.Logger.Information(nameof(HTTPCache), $"{nameof(BeginReadContent)} - Couldn't acquire read lock!", context);
                return null;
            }
        ...
        public bool RefreshHeaders(Hash128 hash, Dictionary<string, List<string>> newHeaders, LoggingContext context)
        {
            ...
            if (_database.Update(hash, newHeaders, context))
            {
                ...
                try
                {
                    return TryMergeHeaders(hash, newHeaders, context);
                }
                catch (Exception ex)
                {
        ...
        /// <summary>
        /// Tries to acquire the write lock on the resource with the provided hash and merge the stored headers with the
        /// newly received ones. If it can't acquire the write lock, no header operations will be done.
        /// </summary>
        /// <remarks>Automatically releases the write lock on exit. If the cached entity was already locked, it unlocks it!</remarks>
        /// <param name="hash">A unique hash identifier for the cached response from a <see cref="HTTPCache.CalculateHash(HTTPMethods, Uri)"/> call.</param>
        /// <param name="newHeaders">A dictionary of new headers to replace or merge with existing headers.</param>
        /// <param name="context">Used by the plugin to add an addition logging context for debugging. It can be <c>null</c>.</param>
        /// <returns><c>true</c> if the headers were successfully refreshed; otherwise, <c>false</c>.</returns>
        private bool TryMergeHeaders(Hash128 hash, Dictionary<string, List<string>> newHeaders, LoggingContext context)
        {
            if (HTTPManager.Logger.IsDiagnostic)
                HTTPManager.Logger.Verbose(nameof(HTTPCacheDatabase), $"{nameof(TryMergeHeaders)}({hash}, {newHeaders?.Count})", context);

            if (!_database.TryAcquireWriteLock(hash, null, context))
            {
                HTTPManager.Logger.Information(nameof(HTTPCache), $"{nameof(TryMergeHeaders)} - Couldn't acquire write lock!", context);
                return false;
            }
            ...
            try
            {
                using var headerStream =
                    HTTPManager.IOService.CreateFileStream(GetHeaderPathFromHash(hash),
            ...
            {
                _database.ReleaseWriteLock(hash, 0, true, context);
            }
        ...
        private void MaintainImplementation(ulong contentLength, bool deleteLockedEntries, DateTime now, LoggingContext context)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < _database.MetadataService.Metadatas.Count; ++i)
                {
                    ...
                    // 2025-05-08: added metadata.Lock != LockTypes.Write check to prevent an exception if an entry is marked for deletion while the entry is actively used (a request is writing it).
                    if (isAnyFileMissing || isIncomplete || metadata.LastAccessTime <= deleteOlderDT && metadata.Lock != LockTypes.Write)
                    {
